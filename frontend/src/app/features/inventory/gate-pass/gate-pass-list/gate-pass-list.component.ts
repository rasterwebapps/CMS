...
export class GatePassListComponent implements OnInit, OnDestroy {
  ...
  protected readonly displayedColumns = ['item', 'locationVirtualName', 'partyName', 'direction', 'expectedReturnDate', 'status', 'actions'];
  protected readonly dataSource = new MatTableDataSource<GatePass>([]);
  protected readonly loading = signal(false);
  protected readonly locations = signal<InventoryLocation[]>([]);

  protected locationFilter: number | null = null;
  protected directionFilter: string | null = null;
  ...
  protected goToNew(): void {
    void this.router.navigate(['/inventory/gate-pass/gate-passes/new']);
  }

  protected openPass(pass: GatePass): void {
    void this.router.navigate(['/inventory/gate-pass/gate-passes', pass.id]);
  }

  private loadPage(): void {
    this.loading.set(true);
    this.gatePassService.getPage({
      ...
      error: () => { this.toast.error('Failed to load gate passes'); this.loading.set(false); },
    });
  }
}
