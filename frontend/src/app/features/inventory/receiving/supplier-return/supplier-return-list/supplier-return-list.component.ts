...
export class SupplierReturnListComponent implements OnInit, OnDestroy {
  ...
  protected readonly displayedColumns = ['returnNumber', 'returnDate', 'goodsReceiptId', 'supplierName', 'locationVirtualName', 'status', 'lineCount', 'actions'];
  protected readonly dataSource = new MatTableDataSource<SupplierReturn>([]);
  protected readonly loading = signal(false);
  protected readonly searchValue = signal('');
  protected readonly canRegenerateNumbers = computed(() => this.permissionService.has('INVENTORY_SUPPLIER_RETURN_REGENERATE_NUMBERS'));
  ...
  protected regenerateNumbers(): void {
    ...
    this.returnService.previewRegenerateNumbers().subscribe({
      next: (preview) => {
        this.regeneratingNumbers.set(false);
        if (preview.totalChanged === 0) {
          this.toast.success('Every supplier return number already matches the current sequence — nothing to change');
          return;
        }
        this.dialog.open(ConfirmDialogComponent, {
          data: {
            title: 'Regenerate Supplier Return Numbers?',
            message: `This will reassign numbers for ${preview.totalChanged} supplier return(s). Old numbers stop working immediately — `
          ...
          if (!confirmed) return;
          this.regeneratingNumbers.set(true);
          this.returnService.regenerateNumbers().subscribe({
            next: (result) => {
              this.toast.success(`Regenerated ${result.totalChanged} supplier return number(s)`);
              this.regeneratingNumbers.set(false);
              this.loadPage();
            },
            error: (err) => {
              this.regeneratingNumbers.set(false);
              this.toast.error(err?.error?.message ?? 'Failed to regenerate supplier return numbers');
            },
          });
        });
      },
      error: (err) => {
        this.regeneratingNumbers.set(false);
        this.toast.error(err?.error?.message ?? 'Failed to preview supplier return number regeneration');
      },
    });
  }
