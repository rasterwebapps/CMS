...
- [x] **R2-4.1.4** `RoomAllocation` entity (binding, capacity-consuming; `student`, `hostelRoom` FK, dates, `status`) — creation rejected (not auto-converted) if `Student.studentType != HOSTELER` (conversion, R2-M4.0, must already be done) — **shipped 2026-07-22.** Migration V309. `RoomAllocationService.create` gates on `studentType == HOSTELER`, rejects a second concurrent `ACTIVE` allocation per student, and checks occupancy (`COUNT(ACTIVE) < HostelRoomType.sharingCapacity`) before allowing a new one. Frontend: `room-allocations` occupancy-map dashboard (room cards grouped/filterable by room type and zone, occupancy bar, allocate flyout with a debounced HOSTELER-only student search, per-occupant cancel). **Explicitly not wired in this pass:** the R2-4.0.2/R2-4.2 fee-recalculation trigger on allocate/cancel — that stays blocked on the billing-engine finding above; RoomAllocation only reads `studentType`, it doesn't touch fee recalculation.

**Gender-restriction cross-check — closed 2026-07-22 (filter, not a block):** decided policy is to filter the *options shown*, not reject an already-picked mismatch after the fact. `MALE`→`BOYS`-restricted or unrestricted (`null`) zones only, `FEMALE`→`GIRLS`-restricted or unrestricted zones only; `GenderRestriction` has no `OTHER` counterpart, so `OTHER`-gender students can only be offered unrestricted (`null`) zones — if none exist yet, staff must create one (documented as an inline hint rather than guessed). Applied in two places: (1) `RoomPreferencePickerComponent` now takes a `gender` input (wired from `form.get('gender')?.value` in both the Enquiry form and both Admission-form branches) and filters its Zone dropdown accordingly, with a hint shown if zero zones match; (2) the Room Allocation dashboard adds a Gender filter dropdown (alongside the existing Room Type/Zone filters) plus a Boys/Girls badge on each room card — `HostelRoomOccupancyResponse` now carries `genderRestriction` for this. No backend-side hard block was added on `RoomAllocationService.create()` itself — the decision was that filtering the candidate pool is sufficient, not defense-in-depth validation at the write path.
