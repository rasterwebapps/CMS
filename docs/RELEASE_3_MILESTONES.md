...
|-----------|--------|----------|
| R3-M1: Core data model | 🟡 In Progress — Category/Uom/Product/CategoryAttribute masters (now including ProductImage photo upload) + InventoryLocation/StockBatch/StockLedger/StockBalance + CycleCount (physical stock counts) shipped; only the deferred `InventoryItem` migration (R3-M8) is left in Phase 1 | 60% |
...
| R3-M4: Requisition & Issue | ✅ Done — Stock Indent (renamed 2026-09-21 from Stock Issue Request), Internal Return, Loanable Item Issue, and Auto-Indent (reopened 2026-09-21 with real user policy input — per-location reorder config, nightly/on-demand detection, two-step department-head/store-fulfillment lifecycle) all shipped (see DECISION_LOG.md) | 100% |
...
| R3-M7: Reporting | ✅ Done — Inventory Dashboard, Stock Valuation Report, PO Aging Report, PO Cycle-Time Report, Asset Depreciation Summary, Price Comparison, and Budget vs. Actual all shipped (Cycle-Time was originally deferred as future work but was picked up as a follow-on in the same session) | 100% |
