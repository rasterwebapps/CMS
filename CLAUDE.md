...
- **Batches are created by Capacity Auto-Plan alone (hard gate).** A `Batch` may only ever be created by `CohortRoomAllocationService.createVentureBatch` (the Capacity Auto-Plan commit flow), which always sets `cohortRoomAllocation`, `cohortSection`, and exactly one of `lab`/`clinicalVenue`. There is no other legitimate creation path.
  1. There is no manual "create batch" affordance anywhere in the app and there must never be one again — OC-191 removed the UI button (`batch-manage-dialog`), and the `POST /batches` endpoint / `BatchService.createBatch` / the frontend `BatchService.create()` method were removed entirely as dead code in the same pass, since nothing legitimate called them. If a future request wants a way to create a batch outside Capacity Auto-Plan, that is a **deviation** — trigger full specialist review before adding it back, don't just wire the button back up.
  2. `Batch.cohortRoomAllocation` (nullable FK) is the only signal distinguishing a real batch from stale/legacy data: non-null means it came from Capacity Auto-Plan; null means it predates this gate and is stale. `Manage Batches` and every other batch picker (Clinical Shift Group, Escort Rotation, Lab Schedule) must only ever surface batches with a non-null `cohortRoomAllocation` going forward.
  3. `BatchService.updateBatch`/`deactivateBatch` (rename/recapacity/reassign-coordinator/deactivate an existing batch) and the roster endpoints remain — this gate is about **creation** only, not managing a batch's lifecycle afterward.
