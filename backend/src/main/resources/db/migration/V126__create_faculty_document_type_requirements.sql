...
-- based on any of: designation, department, or highest qualification.
...
CREATE TABLE faculty_document_type_requirements (
    ...
    CONSTRAINT chk_fdtr_at_least_one_criterion
        CHECK (designation IS NOT NULL OR department_id IS NOT NULL OR qualification IS NOT NULL)
);

CREATE INDEX idx_fdtr_designation   ON faculty_document_type_requirements(designation)   WHERE designation   IS NOT NULL;
CREATE INDEX idx_fdtr_department_id ON faculty_document_type_requirements(department_id) WHERE department_id IS NOT NULL;
