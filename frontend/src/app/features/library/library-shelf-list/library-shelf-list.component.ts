...
import { LibraryShelf, LibraryRack } from '../library.model';
...
@Component({
  selector: 'app-library-shelf-list',
  standalone: true,
  imports: [
  ...
  templateUrl: './library-shelf-list.component.html',
  styleUrl: './library-shelf-list.component.scss',
})
export class LibraryShelfListComponent implements OnInit, OnDestroy {
  ...
  protected readonly dataSource   = new MatTableDataSource<LibraryShelf>([]);
  protected readonly loading      = signal(false);
  protected readonly rackLoading  = signal(true);
  protected readonly searchValue  = signal('');
  protected readonly rack         = signal<LibraryRack | null>(null);
  ...
  protected editShelf(shelf: LibraryShelf): void {
    void this.router.navigate(['/library/racks', this.rackId, 'shelves', shelf.id, 'edit']);
  }

  protected toggleShelfStatus(shelf: LibraryShelf): void {
    const nextAction = shelf.isActive ? 'Deactivate' : 'Activate';
    this.dialog.open(ConfirmDialogComponent, {
      data: {
        title: `${nextAction} Shelf`,
        message: `${nextAction} "${shelf.name}" (${shelf.code})?`,
        confirmText: nextAction,
        cancelText: 'Cancel',
      },
    }).afterClosed().subscribe(confirmed => { if (confirmed) this.performToggle(shelf); });
  }
  ...
  protected handleEmptyAction(): void {
    if (this.searchValue()) {
      ...
      void this.router.navigate(['/library/racks', this.rackId, 'shelves', 'new']);
    }
  }

  private performToggle(shelf: LibraryShelf): void {
    this.libraryService.updateShelfStatus(shelf.id, !shelf.isActive).subscribe({
      next: () => {
        this.toast.success(`Shelf ${shelf.isActive ? 'deactivated' : 'activated'} successfully`);
        this.loadPage();
      },
      error: (err) => {
        this.toast.error(err?.error?.message ?? `Failed to ${shelf.isActive ? 'deactivate' : 'activate'} shelf`);
      },
    });
  }
  ...
  private loadPage(): void {
    ...
    this.libraryService.getShelvesPage({
      ...
      error: () => { this.toast.error('Failed to load shelves'); this.loading.set(false); },
    });
  }
}
