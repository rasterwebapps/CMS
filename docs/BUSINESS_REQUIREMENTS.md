...
|------|----------|-------------------|------------|
| 2026-09-22 | BR-56 | **Conflict Inspector screen removed.** Fully superseded by Timetable Builder's per-cohort "Check & Resolve Conflicts"/Publish gate (OC-260, logged below) — its own term-wide "Proceed to Review" ack fed no other gate (BR-53's term-advance checklist never read it) and its acknowledgment-status endpoint had no remaining frontend caller. Screen, route, nav entry, breadcrumb label, tour, backend controller, and the two now-dead term-wide service methods (`acknowledge`/`getAcknowledgmentStatus`/`isAcknowledgmentValid`) removed; the reused whole-term scan (`scanTerm`) and the surviving per-cohort acknowledgment path are untouched. `TIMETABLE_CONFLICT_INSPECTOR_VIEW`/`_ACKNOWLEDGE` permissions retired via V550; `term_instances.conflict_acknowledged_at`/`conflict_acknowledged_cell_count` (V528) left in place, unused. Evidence: `docs/timetable/DECISION_LOG.md`. | — |
| 2026-09-22 | BR-56 | **Skeleton Builder renamed to Timetable Builder; a real Pending status; attendance-gated actions.** Following a specialist round, the screen, route (`/timetable/timetable-builder`), component/service/model/dialog files and class names, nav entry, tour, and every user-facing string were renamed from Skeleton Builder to Timetable Builder (`TIMETABLE_SKELETON_*` permission keys deliberately kept as-is — no Role Management impact). The cohort status table's lifecycle is now one unified field (`CohortTermStatusSummary.status`, replacing the old separate coarse `status` + `readinessStatus` pair) driving both the badge and the row action: **Pending** (new — a cohort with zero sessions placed yet, which the table already showed by default alongside every other cohort) → **Drafted** → **Conflicts Resolved** → **Published** / **Partially Published**. Discarding a draft returns a cohort to Pending (zero cells); re-running automation on it produces Drafted again; a clean conflict check flips it to Conflicts Resolved with a Publish button — all falling out of the existing gate checks once a cohort with no cells is classified Pending instead of (incorrectly) Drafted. A Pending row gets its own **Run** button, scoping the existing Global Auto-Schedule flyout to just that cohort without opening its grid. New `CohortTermStatusSummary.attendanceRecorded` flag (server-computed from the same attendance guard Discard/Revert-to-Draft already enforced) hides both actions on a row once attendance has been recorded against it, instead of showing a button that would only ever fail server-side. `CmsStatusBadgeComponent.resolveClass()` extended for `PENDING`/`DRAFTED`/`CONFLICTS_RESOLVED` per the badge-consistency gate. No schema change; no new permissions. | — |
| 2026-09-21 | BR-56, BR-57 | **Three named published-timetable views; My Timetable split by audience.** Following a specialist round, the published timetable's Week/Month/Day tabs became **Generic** (the existing recurring-template grid, now correctly hiding its Saturday column for a term with no working-Saturday pattern configured — new `TermInstanceDto.workingSaturdayCount`, `CmsWeekGridComponent.workingSaturdayCount`), **Date-wise-weekly** (new — real dated occurrences for one Mon-Sat calendar week at a time, lazy-loaded per week via the existing `/timetables/occurrences`, paged with the new shared `cms-week-navigator`; shows each date's real HELD/SUBSTITUTED/CANCELLED outcome via `WeekGridSession.occurrenceStatus`), and **Day** (unchanged) — applied identically to `/timetables` (browse-all) and `/my-timetable`. The old calendar-month view/component (`CmsMonthGridComponent`, `shared/calendar-grid/*`) is removed as dead code. `/my-timetable` itself split into two permission-gated screens/nav entries per the operation-wise permission mapping rule — `/my-timetable/student` (`MY_TIMETABLE_VIEW_STUDENT`) and `/my-timetable/staff` (`MY_TIMETABLE_VIEW_STAFF`, retains the Log Progress action the student variant doesn't) — sharing one component distinguished by route `data.audience`, backfilled so no existing `MY_TIMETABLE_VIEW` holder loses capability (V541). Also fixed in the same pass: `WorkingSaturdayCalculator.enabledWorkingSaturdayCount()` corrects a latent bug where Skeleton Builder's own Saturday-column gate (`SkeletonBuilderResponse.workingSaturdayCount`) never actually evaluated to 0 for an unconfigured term, since the raw calculator returns the term's total calendar Saturdays rather than 0 when no working-Saturday pattern is set. See `docs/RELEASE_1_MILESTONES.md` R1-3.2.9. | — |
| 2026-09-15 | BR-56 | **All cohorts by default; one Run button; confirm before overwriting a draft (OC-227 follow-up).** Skeleton Builder's cohort dropdown now defaults to **All cohorts…** for anyone who can run it globally (first individual cohort otherwise, unchanged). The toolbar has one Run button in a fixed spot on the right, relabeling itself **Run Global Auto-Schedule** or **Run Automation** by scope instead of two separately-placed buttons; the All-cohorts empty-state card drops its own duplicate button. Clicking it now confirms first whenever the target scope (a cohort, or any cohort in the term for an All-cohorts run) already has a draft session placed — pinned or not — since every run's rebuild clears everything not pinned and starts over; Cancel changes nothing, confirming opens the existing prerequisite checklist as before. New read-only check `GET /timetables/skeleton/global-auto-place/has-existing-draft`. No schema change. Evidence: `docs/timetable/DECISION_LOG.md`. | — |
| 2026-09-15 | BR-56 | **Switched-off sessions aren't part of the timetable; a run can't double-book a lab (bug fix).** Every Global Auto-Schedule rebuild switches the previous draft off instead of deleting it; the Conflict Inspector scanned those leftovers and reported each rebuilt session as clashing with itself, which also blocked Approve. The Inspector, Approve and revert-to-draft, the unstaffed-sessions list, the draft and published views, the live calendar, faculty workload hours and the term checklist's draft count now ignore switched-off sessions (Discard still deletes everything). Separately, a run could put two batches in one lab at the same time, because its in-memory copy of the week didn't record the lab committed at staffing; it now does, so a lab holds one batch per slot. The Inspector's summary card no longer collapses and hides its counts. No schema change; leftover rows are kept. Evidence: `docs/timetable/DECISION_LOG.md`. | — |
| 2026-09-15 | BR-56 | **Library is one session a week; a second only when the week has room (OC-227 follow-up).** Corrects the Sports entry below: Library's required quota is **1 × 2 periods** per section a week, not 2 × 2. After Sports, Global Auto-Schedule adds one bonus Library session on another day only when the section's cohort still has at least 8 free periods a week (a free Saturday period counts for the share of weeks it really runs); the 8 is the admin setting `timetable.library_extra_session_min_free_periods`. A bonus that isn't placed is never reported. V512 (quota 2 → 1, guarded on the seed value; new setting). Evidence: `docs/timetable/DECISION_LOG.md`. | — |
| 2026-09-15 | BR-56 | **Move and swap whole blocks; move a Clinical duty day (OC-227 follow-up).** In Skeleton Builder a session moves with its whole block (parallel Lab batches, rotation partner and idle-batch fallback included): dropped on empty periods it moves, dropped on occupied periods it swaps with everything in a same-length window, which takes its old periods in the same order. Automation's rules apply (consecutive periods, never across lunch or off the end of the day, blocked periods, duty windows, audience, faculty and room clashes); refused windows say why and legal ones are highlighted while dragging and listed under **Move or swap…**; a before → after preview is confirmed first; all-or-nothing, pinned, draft only. Dragging a Clinical duty banner to another day swaps that day's sessions inside the duty window into the day it leaves, after checking blocked days, other duties, clinical venue capacity and coordinators — new permission `TIMETABLE_SKELETON_DUTY_DAY_MOVE` (V506). Evidence: `docs/timetable/DECISION_LOG.md`. | — |
| 2026-09-15 | BR-56 | **Sports, required filler first, no manual Rotation/Elective Block (OC-227 follow-up).** After curriculum hours, Global Auto-Schedule places the required Library (2 × 2 periods) and a new **Sports** session (1 × 2 periods per section a week, in the day's last periods where free), taught by a PE faculty ticked on the seeded Sports subject's eligible-faculty list and held in a room tagged Sports & Recreation; Self-Study gets exactly its curriculum hours. Leftover periods go equally to the other curriculum Theory subjects, never to Self-Study. Library/Sports that don't fit, or a missing Sports room/PE faculty, are grey notes. **Set up Rotation** and **Place Elective Block** are removed from Skeleton Builder (APIs kept); institution-decided electives now move, swap and are replaced like any subject. The run report's Saturday alert now raises only for real curriculum shortfalls. V505 (Sports subject + settings, widened session-shape check). Evidence: `docs/timetable/DECISION_LOG.md`. | — |
| 2026-09-15 | BR-56 | **Plan against the term's total hours (OC-227 follow-up).** Every subject budget, the placement cap and Global Auto-Schedule now count how many times a placed session really runs in the term — every week on Monday–Friday, only the chosen working Saturdays on Saturday (26 × 5 × 8 weekday periods plus 6 × 8 on first Saturdays). A subject is placed once its sessions cover its curriculum hours, whatever days they sit on, so a first-Saturday session no longer counts as a full weekly session and leaves the subject short. The Saturday hours notice is removed. The replace dialog and displaced-subject strip show hours. No schema change. Evidence: `docs/timetable/DECISION_LOG.md`. | — |
| 2026-09-15 | BR-56 | **Skeleton Builder hours cards measure like with like (OC-227 follow-up).** *Total Hours Available* is now every working day's clock time — all periods plus Clinical duty time outside them (a 07:00 duty starts before Period 1), overlaps counted once, bus buffer excluded, Saturday only on working Saturdays. *Assigned* is curriculum hours capped per subject, with hours beyond a subject's curriculum (the extra-hours filler) shown as "+N h extra", and *Unassigned* is summed per subject so one subject's extra can't mask another's gap. Previously Available counted only period time while Clinical counted full duty length, so Assigned (979.3 h) could exceed Available (906.7 h). Frontend only. |
| 2026-09-15 | BR-56 | **Chosen working Saturdays are regular days (OC-227 follow-up).** When a term has any working-Saturday pattern chosen, Global Auto-Schedule treats Saturday exactly like Monday–Friday for Theory, Lab, Clinical and Library and spreads sessions evenly across Monday–Saturday. Replaces the 2026-09-11 "Saturday is a last resort" ordering and removes the Saturday-upgrade mending pass. On partial patterns the Saturday hours notice reports the hours gap. No schema change. |
| 2026-09-11 | BR-56 | **Monday–Friday first (OC-227).** Global Auto-Schedule now plans every curriculum hour into Monday–Friday first. **Placement order:** a fresh weekday, then backtrack, then a same-day double on a used weekday, then Saturday as a regular day, and only on a term with working Saturdays. **Filler:** Library and extra-hours filler may only use a Saturday that curriculum has already had to open. **Library** shrinks to fit a full week, reported as a neutral note. **Management-selected electives** (*Institution decided*) are timetabled as a common whole-cohort subject in the section's own classroom; unselected options are never scheduled, and a group with no option assigned is flagged. **Clinical duty fit:** the report offers the minimum duty length that closes a roster's Clinical shortfall (e.g. 6 h → 6 h 10 m, at zero period cost) next to the existing extra-duty-days remedy, with an "Apply duty length" button behind the new `TIMETABLE_SKELETON_CLINICAL_DUTY_FIT` (V503). **Saturday notice:** each session that could only go on Saturday is reported with its real hours under the term's pattern. **Report accuracy:** the unplaced list is checked against the finished grid, so sessions that were displaced and later re-placed no longer show. Evidence and specialist decisions: `docs/timetable/DECISION_LOG.md`. | — |
...
1. **Consequence confirmation.** Advancing a term's status now requires confirming a dialog describing what that transition actually does, so an admin isn't guessing. **Upgraded 2026-08-11** from a single warning paragraph to an itemized, system-verified checklist + separate acknowledgment (see the Change Log entry for that date) — each item must be individually ticked, plus one final "I acknowledge this is permanent" checkbox, before the action is enabled:
   - `PLANNED → OPEN`: lists any active cohort with no curriculum version mapped (would silently generate zero offerings for it); a clean pass shows one confirmation row instead.
   - `OPEN → LOCKED`: shows live outstanding fee demand count/amount and unapproved (draft) timetable session count for the term, plus a plainly-labeled self-attestation row for exam results (no system signal exists for that anywhere in the codebase). None of these are hard-blocking on their own — see the Change Log entry for why.
...
- `academic-year-form.component.ts`'s `advanceTermStatus()` — originally wrapped with the shared `ConfirmDialogComponent`; **since 2026-08-11** opens `TermAdvanceChecklistDialogComponent` instead (own component under `features/academic-year/term-advance-checklist-dialog/`), backed by `GET /term-instances/{id}/advance-checklist` and `TermInstanceService.getAdvanceChecklist()`.
...
- **Frontend:** Timetable Builder, Staffing, Capacity Planner (incl. Faculty Workload tab), Faculty Workload Rules screen — all under the Academics nav group. The manual Rotation Setup and Place Elective Block flyouts were removed on 2026-09-15: Run Automation builds Lab rotations and places elective groups itself (`RotationGroupController` and the elective-place endpoint remain).
...
| `TIMETABLE_SKELETON_MANAGE` | Timetable Builder place/remove cell, suggest-candidates (permission key kept as `TIMETABLE_SKELETON_*` through the 2026-09-22 rename) | V336 |
| `TIMETABLE_SKELETON_MOVE` | Timetable Builder drag-and-drop move | V371 |
...
| `TIMETABLE_SKELETON_CLINICAL_DUTY_FIT` | Global Auto-Schedule report's "Apply duty length" — sets a Course Offering's clinical shift duration to the minimum that closes its Clinical hours (OC-227). Defaults to holders of `TIMETABLE_SKELETON_GLOBAL_AUTO_PLACE` | V503 |

### Monday–Friday Scheduling Rules (OC-227, 2026-09-11)

Global Auto-Schedule plans every curriculum hour into the term's working days. With no working Saturdays chosen, that is Monday–Friday only. **Chosen working Saturdays are regular working days (2026-09-15):** once an admin opts the term into any Saturday pattern (1st, 2nd, any nth, or all of them) on Configure Working Saturdays, Saturday is treated exactly like Monday–Friday for Theory, Lab, Clinical and Library, and sessions are spread evenly across Monday–Saturday, so the chosen Saturdays are genuinely occupied. One-off Special Classes (BR-55) remain the tool for short-notice catch-up or portions a teacher couldn't finish.

1. **Placement order for each weekly session:**
   1. A fresh working day (one session per subject per day), least-loaded day first.
   2. Backtrack (bump another row to make room).
   3. A second session on a working day the subject already uses (back-to-back allowed).
   4. Report it as unplaced.

   Saturday is one of the working days in every step when the term has chosen working Saturdays; it is never a separate fallback.
2. **Filler uses the same working days, required first (2026-09-15).** After curriculum, the working-day periods curriculum left free go first to the required quotas — Library (1 × 2 periods per section a week) and **Sports** (1 × 2 periods per section a week, in the day's last periods where free, taught by a PE faculty ticked on the Sports subject's eligible-faculty list, in a room tagged Sports & Recreation) — and Self-Study/Co-curricular gets exactly its curriculum hours. A section whose week still has 8 or more free periods after that gets one bonus Library session on another day (the admin setting `timetable.library_extra_session_min_free_periods`; a free Saturday period counts for the share of weeks it really runs). Whatever is left goes to the cohort's other non-elective curriculum Theory subjects as extra revision, in equal amounts; Self-Study takes no extra. Saturday is included when the term has chosen working Saturdays.
3. **Library and Sports yield to curriculum.** When a cohort's week is full, Library and Sports shrink, e.g. Library to 0 of its 1 weekly session. The run report shows this as a neutral note, not a warning — as it does a missing Sports room or PE faculty. A bonus Library session that isn't placed is not reported at all. A missing Library classroom is still a warning.
4. **Management-selected electives are common cohort subjects.**
   - This applies to groups set to *Institution decided* on Elective Assignment. The option the cohort's students are bulk-assigned to is timetabled as an ordinary Theory subject for the whole cohort.
   - It is taught in the section's own committed classroom, under normal cohort-exclusivity rules.
   - Unselected options are never scheduled, and a group with no option assigned yet is flagged.
   - Student-choice groups keep the shared group slot, but once registrations exist, options with no students are skipped.
   - In the grid, the elective renders as a regular cell with a small "Elective" tag.
5. **Clinical duty fit.** A shift roster can land just short of a subject's curriculum Clinical hours (a 6 h duty × 26 weeks = 156 h against a 160 h unit). When it does, the run report offers two remedies:
   - the existing one, extra duty days on a date-bounded shift group;
   - new: the minimum duty length that closes the gap (e.g. 6 h → 6 h 10 m), with a flag for whether it costs any timetable period.

   "Apply duty length" saves it to the Course Offering. The minutes are recomputed server-side, and the offering's existing floor validation still applies.
6. **Hours are planned against the term's total (2026-09-15).** Available capacity is every working day's periods across the term. For example, 8 periods over 26 weeks gives 26 × 5 × 8 = 1,040 weekday periods, and "1st Saturday only" adds 6 × 8 = 48.
   - A placed session is credited with how many times it really runs: every week on a weekday, only on the chosen working Saturdays on Saturday. A 50-minute first-Saturday session adds 5 h, not 21.7 h.
   - A subject counts as placed once its sessions cover its curriculum hours, whatever days they sit on. The placement cap (manual and automated) refuses a further session only once the hours are met.
   - Weekday and Saturday are the same kind of capacity, so there is no Saturday notice. Only a genuine shortfall is reported, where the term's periods can't hold the hours.
   - Replaces the earlier "Saturday hours notice".
7. **An honest run report.** The per-cohort unplaced list is checked against the finished grid. An item is dropped once its subject's sessions covered its hours by the end of the run, for example a session "displaced during a backtrack" and later re-placed.

### Migration Notes

V265, V267 (elective groups) · V271–V274 (batches) · V293 (`class_schedules` rename/generalize) · V294 (base timetable permissions) · V313–V315 (faculty availability, swap/discard permissions — full detail in BR-57) · V322 (`session_occurrences`) · V331 (merge LabSlot into shared `Period` master) · V332–V334 (clinical venues, CLINICAL session type) · V335 (unstaffed DRAFT rows) · V336 (skeleton permission) · V345–V348 (clinical capacity, capacity planner permissions, blocked periods) · V358–V361 (room-hierarchy linkage, cohort room allocations, allocation permissions) · V363–V368 (rotation groups, cohort sections, batch/class-schedule section links) · V369 (informational-only `secondaryFacultyId`) · V370 (workload-cap config rows) · V371–V372 (skeleton-move, auto-place/auto-staff permissions) · V373 (`default_weekly_teaching_hours`/`planned_weekly_hours_override`, OC-119) · V377 (Faculty Workload Rules permissions, OC-122) · V503 (`TIMETABLE_SKELETON_CLINICAL_DUTY_FIT`, OC-227).
...
- **No print/export capability anywhere in the timetable module** — greenfield, not built. Decided via @Partner specialist review (2026-09-22, tracked as OC-262): a new 4th screen, **Consolidated** (weekly Day×Period wall-chart grid layout, matching print/notice-board/compliance-inspection needs), sits alongside the 3 existing interactive views; all 4 views (Timetable browse view, My Timetable, Resource Timetable Grid, and Consolidated) get their own self-service Export action (PDF grid / Excel flat table), each gated by its own dedicated `*_EXPORT` permission per the operation-wise permission hard gate. No export audit logging — the server-rendered print date on the document was judged sufficient. Open question not yet resolved: whether Consolidated is single-section-per-page only, or also bundles multiple sections/a whole department into one export.
