...
public class LibraryBookExportService {
    ...
    private static final String[] HEADERS = {
        "#", "Acc. No.", "Title", "Author(s)", "Publisher", "Shelf", "Call No.", "Status",
    };

    // ── Excel ─────────────────────────────────────────────────────────────────

    public byte[] toExcel(List<LibraryBookResponse> rows) throws IOException {
        try (XSSFWorkbook wb = new XSSFWorkbook();
            ...
            for (LibraryBookResponse b : rows) {
                ...
                setCell(row, 1, nvl(b.accessionNumber()), style);
                setCell(row, 2, nvl(b.title()), style);
                setCell(row, 3, nvl(b.authors()), style);
                setCell(row, 4, nvl(b.publisher()), style);
                setCell(row, 5, shelfLabel(b), style);
                setCell(row, 6, nvl(b.callNumber()), style);
                setCell(row, 7, b.status() != null ? b.status().name() : "—", style);

                rowIdx++;
            }

            int[] widths = { 6, 16, 34, 24, 22, 18, 16, 14 };
    ...
    public byte[] toPdf(List<LibraryBookResponse> rows) throws IOException {
        try (ByteArrayOutputStream out = new ByteArrayOutputStream()) {
            ...
            float[] colWidths = { 4, 10, 22, 16, 15, 12, 11, 9 };
            ...
            for (LibraryBookResponse b : rows) {
                ...
                addCell(table, nvl(b.accessionNumber()), dataFont, rowBg, Element.ALIGN_LEFT);
                addCell(table, nvl(b.title()), dataFont, rowBg, Element.ALIGN_LEFT);
                addCell(table, nvl(b.authors()), dataFont, rowBg, Element.ALIGN_LEFT);
                addCell(table, nvl(b.publisher()), dataFont, rowBg, Element.ALIGN_LEFT);
                addCell(table, shelfLabel(b), dataFont, rowBg, Element.ALIGN_LEFT);
                addCell(table, nvl(b.callNumber()), dataFont, rowBg, Element.ALIGN_LEFT);
                addCell(table, b.status() != null ? b.status().name() : "—", dataFont, rowBg, Element.ALIGN_LEFT);
                idx++;
            }
