...
- [x] **R1-3.2.9** Create lab calendar view (weekly timetable grid) — **resolved 2026-07-21:** confirmed covered by the existing `/timetables` browse view (`timetable-view.component.ts`), which already renders the shared `CmsWeekGridComponent` week-grid. `/lab-schedules` and `/timetables` read the same `class_schedules` table (renamed from `lab_schedules` in V293) — `/lab-schedules` is the full CRUD management screen (all rows, draft + published, THEORY + LAB), `/timetables` is the read-only PUBLISHED-only calendar browse/personal view. Decision: no new calendar UI needed on the management screen; `/timetables` already satisfies the "weekly timetable grid" requirement. `lab-schedule-list.component.ts` remains a plain table by design — that's the CRUD view, not the calendar view. **Superseded 2026-09-21:** a follow-up specialist round asked for three named views rather than the original ad hoc Week/Month/Day — Generic (the recurring template grid this item resolved, now correctly hides Saturday for a 5-day term via `TermInstanceDto.workingSaturdayCount`), Date-wise-weekly (new — real dated occurrences one Mon-Sat week at a time via `cms-week-navigator`, lazy-loaded per week, replacing the old calendar-month view), and Day (unchanged). Applied identically to `/timetables` (browse-all) and to `/my-timetable`, which itself split into two permission-gated screens/nav entries, `/my-timetable/student` (`MY_TIMETABLE_VIEW_STUDENT`) and `/my-timetable/staff` (`MY_TIMETABLE_VIEW_STAFF`, keeps the Log Progress action) — see V541 migration. Also fixed in the same pass: `WorkingSaturdayCalculator.enabledWorkingSaturdayCount()` corrects a latent bug where Skeleton Builder's own Saturday-column gate (`SkeletonBuilderResponse.workingSaturdayCount`) never actually evaluated to 0 for an unconfigured term, since the raw calculator returns the term's total calendar Saturdays rather than 0 when no working-Saturday pattern is set.
