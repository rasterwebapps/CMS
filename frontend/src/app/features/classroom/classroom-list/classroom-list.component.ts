...
export class ClassroomListComponent implements OnInit, OnDestroy {
  ...
  protected editClassroom(item: Classroom): void {
    void this.router.navigate(['/classrooms', item.id, 'edit']);
  }

  protected toggleClassroomStatus(item: Classroom): void {
    const nextAction = item.isActive ? 'Deactivate' : 'Activate';
    this.dialog.open(ConfirmDialogComponent, {
      data: {
        title: `${nextAction} Classroom`,
  ...
  protected handleEmptyAction(): void {
    if (this.searchValue()) {
      ...
      void this.router.navigate(['/classrooms/new']);
    }
  }
  ...
  private performToggle(item: Classroom): void {
    this.loading.set(true);
    this.classroomService.updateStatus(item.id, { isActive: !item.isActive }).subscribe({
      next: () => {
        this.toast.success(`Classroom ${item.isActive ? 'deactivated' : 'activated'} successfully`);
        this.loadPage();
      },
      error: (err) => {
        this.toast.error(
          err?.error?.message ?? `Failed to ${item.isActive ? 'deactivate' : 'activate'} classroom`,
        );
  ...
  private loadPage(): void {
    ...
    this.classroomService.getPage({ search, page: this.currentPage, size: this.currentPageSize, sort: this.sortActive, direction: this.sortDirection }).subscribe({
      next: (page) => {
      ...
      error: () => { this.toast.error('Failed to load classrooms'); this.loading.set(false); },
    });
  }
}
