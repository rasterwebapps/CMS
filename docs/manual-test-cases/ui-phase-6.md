...
2. Click the "Specialities" link in the sidenav.
...
- A test speciality exists

**Steps:**
1. Navigate to `/specialities`.
2. Click "Add Speciality".
...
6. Trigger an error toast (e.g. delete a speciality that's referenced elsewhere).
...
- On success: button briefly turns green with a white checkmark animating in (~200ms), then the page navigates to `/specialities` after ~600ms.
