...
**Doing now:** Suppliers can now be registered, approved, and managed, with a standing rate agreement (Rate Contract) tracked per supplier, a small named-tax-rate list (Tax Rule) purchase orders will use later, a product can now be linked to a supplier with its own rate (Vendor Product Rate) — optionally overridden by a negotiated per-product rate line on that supplier's Rate Contract while it's active — and a location can now submit a Purchase Requisition (a request to buy specific products), reviewed and approved or rejected per product. Next up: a reorder-triggered "wanted list" (auto-computed shortages) and the actual Purchase Order screen an approved requisition line feeds into.

**Todo:**
- Purchase request → purchase order flow — Purchase Requisition done (request + per-line approve/reject); reorder-triggered shortage list and Purchase Order itself still to come
