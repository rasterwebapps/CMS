...
export class HostelRoomTypeListComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  protected readonly displayedColumns = ['code', 'name', 'sharingCapacity', 'isAc', 'feeAmountPerYear', 'isActive', 'actions'];
  protected readonly dataSource = new MatTableDataSource<HostelRoomType>([]);
  ...
  private readonly sortMap: Record<string, string> = {
    name: 'name', code: 'code', sharingCapacity: 'sharingCapacity',
    feeAmountPerYear: 'feeAmountPerYear', isActive: 'isActive',
  };
  ...
  protected editRoomType(item: HostelRoomType): void {
    void this.router.navigate(['/hostel-room-types', item.id, 'edit']);
  }

  protected toggleRoomTypeStatus(item: HostelRoomType): void {
    const nextAction = item.isActive ? 'Deactivate' : 'Activate';
    this.dialog.open(ConfirmDialogComponent, {
      data: {
        title: `${nextAction} Hostel Room Type`,
  ...
  protected handleEmptyAction(): void {
    if (this.searchValue()) {
      ...
      void this.router.navigate(['/hostel-room-types/new']);
    }
  }
  ...
  private performToggle(item: HostelRoomType): void {
    this.loading.set(true);
    this.roomTypeService.updateStatus(item.id, { isActive: !item.isActive }).subscribe({
      next: () => {
        this.toast.success(`Hostel room type ${item.isActive ? 'deactivated' : 'activated'} successfully`);
        this.loadPage();
      },
      error: (err) => {
        this.toast.error(
          err?.error?.message ?? `Failed to ${item.isActive ? 'deactivate' : 'activate'} hostel room type`,
        );
  ...
  private loadPage(): void {
    ...
    this.roomTypeService.getPage({ search, page: this.currentPage, size: this.currentPageSize, sort: this.sortMap[this.sortActive] ?? this.sortActive, direction: this.sortDirection }).subscribe({
      next: (page) => {
      ...
      error: () => { this.toast.error('Failed to load hostel room types'); this.loading.set(false); },
    });
  }
}
