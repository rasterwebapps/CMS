...
import { ConsignmentAgreement } from '../agreement.model';
...
@Component({
  selector: 'app-consignment-agreement-list',
  standalone: true,
  imports: [
    FormsModule,
    DatePipe,
...
export class ConsignmentAgreementListComponent implements OnInit, OnDestroy {
  ...
  protected readonly displayedColumns = ['supplierName', 'locationVirtualName', 'agreementNumber', 'period', 'billingCycleDays', 'actions'];
  protected readonly dataSource = new MatTableDataSource<ConsignmentAgreement>([]);
  ...
  protected goToNew(): void {
    void this.router.navigate(['/inventory/consignment/agreements/new']);
  }

  protected editAgreement(item: ConsignmentAgreement): void {
    void this.router.navigate(['/inventory/consignment/agreements', item.id, 'edit']);
  }

  private loadPage(): void {
    this.loading.set(true);
    this.agreementService.getPage({
      ...
      error: () => { this.toast.error('Failed to load consignment agreements'); this.loading.set(false); },
    });
  }
}
