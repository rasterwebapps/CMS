...
import { ClinicalResidualItem, ClinicalShiftDayShortfall, FacultyOverCapacity, FacultySubstitutionTip, FacultyTightCapacity, GlobalAutoSchedulePrerequisites, GlobalAutoScheduleResult, VenueCapacityGap, VenueOverCapacity, VenueTightCapacity } from './timetable-builder.model';
...
export class GlobalAutoScheduleReportFlyoutComponent implements OnInit {
  ...
  /** Whether this run was launched from inside a single cohort's own grid (as opposed to the "All
   *  cohorts" table) — OC-265: the success step's own closing button says "review cohorts below"
   *  because closing normally lands back on that table's row list; from inside a cohort's grid
   *  there's no such list to land on (you're already looking at that cohort's now-populated grid),
   *  so the wording there would be actively wrong rather than just imprecise. */
  readonly fromGridView = input(false);
