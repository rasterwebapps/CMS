...
import { UomRequest } from '../uom.model';
...
export class UomFormComponent implements OnInit {
  ...
  protected readonly pageTitle  = signal('Add Unit of Measure');

  protected readonly previewName   = signal('');
  protected readonly previewCode   = signal('');
  protected readonly codeCharCount = signal(0);

  private uomId: number | null = null;
  ...
  ngOnInit(): void {
    ...
    if (idParam) {
      this.uomId = Number(idParam);
      this.isEditMode.set(true);
      this.pageTitle.set('Edit Unit of Measure');
      this.loadUom();
    }
  ...
  private setupUniquenessValidators(): void {
    ...
    nameCtrl?.setAsyncValidators(
      uniqueFieldValidator(this.http, `${environment.apiUrl}/inventory/uoms/name-exists`, () => this.uomId),
    );
    nameCtrl?.updateValueAndValidity({ emitEvent: false });

    const codeCtrl = this.form.get('code');
    codeCtrl?.setAsyncValidators(
      uniqueFieldValidator(this.http, `${environment.apiUrl}/inventory/uoms/code-exists`, () => this.uomId),
    );
  ...
  protected onSubmit(): void {
    ...
    const op$ = this.isEditMode()
      ? this.uomService.update(this.uomId!, request)
      : this.uomService.create(request);

    op$.subscribe({
      next: () => {
        this.toast.success(this.isEditMode() ? 'Unit of measure updated successfully' : 'Unit of measure created successfully');
        this.saving.set(false);
        void this.router.navigate(['/inventory/uoms']);
      },
      error: (err) => {
        this.toast.error(err?.error?.message ?? (this.isEditMode() ? 'Failed to update unit of measure' : 'Failed to create unit of measure'));
        this.saving.set(false);
      },
    });
  }
  ...
  protected getErrorMessage(fieldName: string): string {
    return cmsFieldError(this.form.get(fieldName), UomFormComponent.FIELD_LABELS[fieldName] ?? fieldName);
  }

  private loadUom(): void {
    if (!this.uomId) return;
    this.loading.set(true);
    this.uomService.getById(this.uomId).subscribe({
      next: (u) => {
      ...
      error: () => {
        this.toast.error('Failed to load unit of measure');
        void this.router.navigate(['/inventory/uoms']);
      },
    });
  }
