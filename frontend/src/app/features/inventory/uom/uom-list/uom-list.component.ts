...
export class UomListComponent implements OnInit, OnDestroy {
  ...
  private readonly VIEW_MODE_KEY = 'uom-view-mode';
  ...
  protected readonly displayedColumns = ['code', 'name', 'isActive', 'actions'];
  protected readonly dataSource = new MatTableDataSource<Uom>([]);
  ...
  protected editUom(item: Uom): void {
    void this.router.navigate(['/inventory/uoms', item.id, 'edit']);
  }

  protected toggleUomStatus(item: Uom): void {
    const nextAction = item.isActive ? 'Deactivate' : 'Activate';
    this.dialog.open(ConfirmDialogComponent, {
      data: {
        title: `${nextAction} Unit of Measure`,
  ...
  protected handleEmptyAction(): void {
    if (this.searchValue()) {
      ...
      void this.router.navigate(['/inventory/uoms/new']);
    }
  }
  ...
  private performToggle(item: Uom): void {
    this.loading.set(true);
    this.uomService.updateStatus(item.id, { isActive: !item.isActive }).subscribe({
      next: () => {
        this.toast.success(`Unit of measure ${item.isActive ? 'deactivated' : 'activated'} successfully`);
        this.loadPage();
      },
      error: (err) => {
        this.toast.error(err?.error?.message ?? `Failed to ${item.isActive ? 'deactivate' : 'activate'} unit of measure`);
  ...
  private loadPage(): void {
    ...
    this.uomService.getPage({
      ...
      error: () => { this.toast.error('Failed to load units of measure'); this.loading.set(false); },
    });
  }
}
