...
export class StockTransferListComponent implements OnInit, OnDestroy {
  ...
  protected readonly displayedColumns = ['transferDate', 'sourceLocationVirtualName', 'destinationLocationVirtualName', 'status', 'lineCount', 'actions'];
  protected readonly dataSource = new MatTableDataSource<StockTransfer>([]);
  ...
  protected openTransfer(item: StockTransfer): void {
    void this.router.navigate(['/inventory/stock/transfers', item.id]);
  }
  ...
  private loadPage(): void {
    ...
    this.transferService.getPage({
      ...
      error: () => { this.toast.error('Failed to load stock transfers'); this.loading.set(false); },
    });
  }
}
