...
export class PeriodListComponent implements OnInit, OnDestroy {
  ...
  protected readonly displayedColumns = ['periodOrder', 'name', 'startTime', 'endTime', 'isActive', 'actions'];
  protected readonly dataSource = new MatTableDataSource<Period>([]);
  ...
  protected sortActive = 'periodOrder';
  ...
  protected editPeriod(item: Period): void {
    void this.router.navigate(['/periods', item.id, 'edit']);
  }

  protected togglePeriodStatus(item: Period): void {
    const nextAction = item.isActive ? 'Deactivate' : 'Activate';
    this.dialog.open(ConfirmDialogComponent, {
      data: {
        title: `${nextAction} Period`,
  ...
  protected handleEmptyAction(): void {
    if (this.searchValue()) {
      ...
      void this.router.navigate(['/periods/new']);
    }
  }
  ...
  private performToggle(item: Period): void {
    this.loading.set(true);
    this.periodService.updateStatus(item.id, { isActive: !item.isActive }).subscribe({
      next: () => {
        this.toast.success(`Period ${item.isActive ? 'deactivated' : 'activated'} successfully`);
        this.loadPage();
      },
      error: (err) => {
        this.toast.error(
          err?.error?.message ?? `Failed to ${item.isActive ? 'deactivate' : 'activate'} period`,
        );
  ...
  private loadPage(): void {
    ...
    this.periodService.getPage({ search, page: this.currentPage, size: this.currentPageSize, sort: this.sortActive, direction: this.sortDirection }).subscribe({
      next: (page) => {
      ...
      error: () => { this.toast.error('Failed to load periods'); this.loading.set(false); },
    });
  }
}
