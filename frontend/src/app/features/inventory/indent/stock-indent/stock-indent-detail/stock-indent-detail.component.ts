...
import { StockIndent, StockIndentItem } from '../stock-indent.model';
...
@Component({
  ...
  templateUrl: './stock-indent-detail.component.html',
  styleUrl: './stock-indent-detail.component.scss',
})
export class StockIndentDetailComponent implements OnInit {
  ...
  protected readonly indent = signal<StockIndent | null>(null);
  ...
  protected load(): void {
    ...
    this.requestService.getById(this.requestId).subscribe({
      next: (r) => { this.indent.set(r); this.loading.set(false); },
      error: () => { this.toast.error('Failed to load stock indent'); this.loading.set(false); },
    });
  }
  ...
  protected removeLine(line: StockIndentItem): void {
    this.busy.set(true);
  ...
  protected rejectLine(line: StockIndentItem): void {
    this.busy.set(true);
  ...
  protected returnLine(line: StockIndentItem): void {
  ...
  protected goBack(): void {
    void this.router.navigate(['/inventory/indent/stock-indents']);
  }
}
