...
export class BrandListComponent implements OnInit, OnDestroy {
  ...
  private readonly VIEW_MODE_KEY = 'brand-view-mode';
  ...
  protected readonly displayedColumns = ['name', 'isActive', 'actions'];
  protected readonly dataSource = new MatTableDataSource<Brand>([]);
  ...
  protected editBrand(item: Brand): void {
    void this.router.navigate(['/inventory/brands', item.id, 'edit']);
  }

  protected toggleBrandStatus(item: Brand): void {
    const nextAction = item.isActive ? 'Deactivate' : 'Activate';
    this.dialog.open(ConfirmDialogComponent, {
      data: {
        title: `${nextAction} Brand`,
  ...
  protected handleEmptyAction(): void {
    if (this.searchValue()) {
      ...
      void this.router.navigate(['/inventory/brands/new']);
    }
  }
  ...
  private performToggle(item: Brand): void {
    this.loading.set(true);
    this.brandService.updateStatus(item.id, { isActive: !item.isActive }).subscribe({
      next: () => {
        this.toast.success(`Brand ${item.isActive ? 'deactivated' : 'activated'} successfully`);
        this.loadPage();
      },
      error: (err) => {
        this.toast.error(err?.error?.message ?? `Failed to ${item.isActive ? 'deactivate' : 'activate'} brand`);
  ...
  private loadPage(): void {
    ...
    this.brandService.getPage({
      ...
      error: () => { this.toast.error('Failed to load brands'); this.loading.set(false); },
    });
  }
}
