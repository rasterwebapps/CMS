...
| 13 | Auto-fit still truncated most `.cell-mono`/currency columns even after double-clicking, worse than plain text columns | `canvas.measureText()` is only ever an *approximation* of real DOM text layout — tried patching it for a specific cause (web-font-loading race for `--cms-font-mono`) and it still didn't fully match reality, pointing at canvas measurement itself being the unreliable part (kerning, letter-spacing, `font-variant-numeric`, synthesized font weights — none of which canvas's simple `font` string reproduces faithfully) | Abandoned canvas measurement entirely. `measureCellWidth()` now clones each leaf into an offscreen unconstrained container and reads its real `getBoundingClientRect().width` — the actual browser layout engine, guaranteed to match the DOM exactly since nothing is approximated |
| 14 | A short, sortable, right-aligned numeric header (`arrowPosition="before"`) stayed truncated even after auto-fit | The Material sort-arrow icon is a separate SVG element sitting in front of the text (`.cell-align-right`'s `.mat-sort-header-arrow { order: -1 }`), not part of any leaf's `textContent` — text measurement alone has no idea it's there or how much space it (+ its margin) takes | `measureCellWidth()` adds the sort-arrow's own `getBoundingClientRect().width` + margins on top of the text measurement when present |
| 15 | A 3-line stacked cell (name/phone/email) stayed visually truncated right after auto-fit, then rendered correctly once an unrelated sort click re-rendered the table | `table-layout: auto` can leave a column rendered at its old width for a beat after the injected `<style>` text has already changed, until something else forces a full layout pass — a sort-triggered row re-render happened to be that trigger, incidentally | `rebuildWidthStyles()` reads `offsetHeight` right after writing the new rule, forcing the browser to fully recompute layout against it immediately instead of leaving that to chance |
| 16 | An avatar+stacked-text cell (`.student-cell`/`.name-cell`) auto-fit to a width that still truncated the email — confirmed via the injected stylesheet's own written number being too small, not a rendering-timing issue | `measureCellWidth()` measured each leaf's own width in isolation, blind to the avatar sitting to its left as a fixed-width flex sibling — its width + the flex `gap` were silently dropped from the total | Add each leaf's horizontal offset from the cell's own left edge (`el.getBoundingClientRect().left - cell.getBoundingClientRect().left`) to its measured width — this captures any preceding sibling (avatar, icon, anything) without needing to know what it is, and stays accurate even when the column is currently very narrow since a `flex-shrink: 0` avatar never shrinks |
| 17 | Items 13–16 kept happening one column pattern at a time — every fix to the leaf-by-leaf reconstruction (per-leaf font, sort-arrow width, leaf offset, a generic padding buffer) fixed the reported case but risked both under- *and* over-sizing others, since the buffers stacked inconsistently depending on which patterns a given cell happened to combine | The fundamental design was wrong: reconstructing a cell's width from isolated pieces (leaf text + manually-added icon widths + manually-added offsets + a padding guess) can never cover every markup combination — there's always another pattern it doesn't know about | **Replaced the whole approach.** `measureUnconstrainedWidth()` clones a cell's *entire* real DOM subtree (not isolated leaves) into an offscreen container and reads the clone's real rendered width directly — avatars, icons, badges, sort arrows, custom components, anything, all included automatically because it's the real markup laid out by the real browser engine, not reconstructed piece by piece. |
| 18 | Every column measured a uniform ~40px too narrow immediately after switching to whole-cell cloning (item 17) — first appeared as under-sizing on plain `.cell-mono` columns that had been correctly sized just before the switch | The shared cell padding (`td.mat-cell` — `styles.scss`) is a **scoped** rule nested inside `.modern-table { }`, requiring a `table.modern-table` ancestor. Once a clone is detached and reparented into the offscreen measurement container, it's no longer inside that table and silently loses its padding — `box-sizing: border-box` only includes padding that's actually *set*, and after detachment, none was | `preserveBoxModelAndStripConstraints()` reads each original element's real computed padding/border/font *before* cloning and pins them onto the clone as explicit inline styles, which always win regardless of where the element ends up — preserving the actual rendered box model instead of hoping a scoped rule still matches post-detachment. Walks original and clone in lockstep so every level of a multi-line/avatar cell gets this, not just the top-level `<td>` |

This is the current, final implementation — items 13–16 are kept for history only.
