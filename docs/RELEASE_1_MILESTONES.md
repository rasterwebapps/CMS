...
**Backend:**
- [x] **R1-2.1.1** Create `Department` entity (`id`, `name`, `code`, `description`, `hodName`, `createdAt`, `updatedAt`)
- [x] **R1-2.1.2** Create `DepartmentRepository` (JpaRepository)
- [x] **R1-2.1.3** Create `DepartmentService` with CRUD operations
- [x] **R1-2.1.4** Create `DepartmentController` — REST endpoints:
...
- [x] **R1-2.1.5** Create `DepartmentRequest` and `DepartmentResponse` DTOs (Java records)
- [x] **R1-2.1.6** Create Flyway migration: `V1__create_departments_table.sql`
- [x] **R1-2.1.7** Write unit + controller tests (95% coverage)
- [x] **R1-2.1.8** Create manual test cases: `docs/manual-test-cases/department-management.md`

**Frontend:**
- [x] **R1-2.1.9** Create `features/department/` folder structure
- [x] **R1-2.1.10** Create `DepartmentService` (API calls)
- [x] **R1-2.1.11** Create `DepartmentListComponent` — Material table with search, sort, pagination
- [x] **R1-2.1.12** Create `DepartmentFormComponent` — Create/Edit form with validation
- [x] **R1-2.1.13** Create department routes (lazy-loaded)
...
**Backend:**
- [x] **R1-2.4.1** Create `Lab` entity (`id`, `name`, `labType`, `department`, `building`, `roomNumber`, `capacity`, `status`)
...
- [x] **R1-2.4.9** Create `LabListComponent` — Material table with filters (by department, type, status)
...
**Backend:**
- [x] **R1-2.5.1** Create `Faculty` entity (`id`, `employeeCode`, `firstName`, `lastName`, `email`, `phone`, `department`, `designation`, `specialization`, `labExpertise`, `joiningDate`)
...
- [ ] **R1-4.1.1a** Create `FeeStructureYearAmount` entity (`id`, `feeStructure`, `yearNumber`, `yearLabel`, `amount` [BigDecimal])
...
- [ ] **R1-4.1.9a** Enhance fee structure form with dynamic year-wise fee input boxes based on program `durationYears` (BR-2)
...
**Backend:**
- [ ] **R1-4.1b.1** Enhance `Enquiry` entity with fee guideline fields:
...
- [ ] **R1-4.1b.4** Create fee finalization endpoints for admin (BR-6):
...
- [ ] **R1-4.1b.5** Enhance payment collection for enquiry-based fees (BR-7)
- [ ] **R1-4.1b.6** Create document submission tracking for enquiries (BR-9):
...
- [ ] **R1-4.1b.10** Write unit + controller tests (95% coverage)
- [ ] **R1-4.1b.11** Update manual test cases: `docs/manual-test-cases/enquiry-management.md`

**Frontend:**
- [ ] **R1-4.1b.12** Enhance enquiry form with fee structure guideline panel (BR-3)
  - Side panel showing fee structure for selected program in current academic year
  - Year-wise breakdown displayed dynamically
- [ ] **R1-4.1b.13** Add referral type dropdown (from master) with conditional additional amount box (BR-4, BR-5)
  - Show additional amount box only when referral type has non-zero guidelineValue
  - Calculate and display final fee
- [ ] **R1-4.1b.14** Create admin fee finalization screen (BR-6)
  - Display enquiry guideline values as starting point
  - Allow admin to adjust fees, provide discounts, modify year-wise split
- [ ] **R1-4.1b.15** Enhance payment collection screen for enquiry-based workflow (BR-7)
- [ ] **R1-4.1b.16** Create document submission tracking component (BR-9)
- [ ] **R1-4.1b.17** Update enquiry list with new status workflow and automatic transitions (BR-8)
- [ ] **R1-4.1b.18** Create student explorer screen with comprehensive filters (BR-11)
