...
ssh_run "
  if [ -f $REMOTE_DIR/.env ]; then
    ...
    if grep -q '^MINIO_ACCESS_KEY=' $REMOTE_DIR/.env; then
      sed -i 's|^MINIO_ACCESS_KEY=.*|MINIO_ACCESS_KEY=$MINIO_ACCESS_KEY|' $REMOTE_DIR/.env
    else
      printf 'MINIO_ACCESS_KEY=$MINIO_ACCESS_KEY\n' >> $REMOTE_DIR/.env
    fi
    if grep -q '^MINIO_SECRET_KEY=' $REMOTE_DIR/.env; then
      sed -i 's|^MINIO_SECRET_KEY=.*|MINIO_SECRET_KEY=$MINIO_SECRET_KEY|' $REMOTE_DIR/.env
    else
      printf 'MINIO_SECRET_KEY=$MINIO_SECRET_KEY\n' >> $REMOTE_DIR/.env
    fi
    chmod 600 $REMOTE_DIR/.env
