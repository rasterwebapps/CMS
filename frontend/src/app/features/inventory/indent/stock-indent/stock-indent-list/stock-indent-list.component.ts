...
import { StockIndent } from '../stock-indent.model';
...
@Component({
  ...
  styleUrl: './stock-indent-list.component.scss',
})
export class StockIndentListComponent implements OnInit, OnDestroy {
  ...
  protected readonly dataSource = new MatTableDataSource<StockIndent>([]);
  ...
  protected goToNew(): void {
    void this.router.navigate(['/inventory/indent/stock-indents/new']);
  }

  protected openRequest(item: StockIndent): void {
    void this.router.navigate(['/inventory/indent/stock-indents', item.id]);
  }
  ...
  private loadPage(): void {
    ...
    this.requestService.getPage({
      ...
      error: () => { this.toast.error('Failed to load stock indents'); this.loading.set(false); },
    });
  }
}
